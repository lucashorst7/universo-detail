...
export default function GuidesPage() {
  ...
  useEffect(() => {
    ...
    fetchGuideBySlug(slug).then(async (g) => {
      setActiveGuide(g)
      if (g) {
        const gps = await fetchGuideProducts(g.id)
        setProducts(gps as GuideProduct[])
      }
    })
  }, [slug])
      ...
      <div className="container page">
        ...
        <div className="page-header">
          <div>
            ...
            {activeGuide.description && <p className="page-subtitle">{activeGuide.description}</p>}
          </div>
        </div>
        {products.length > 0 && (
          <div className="product-grid">
            {products.map((gp) => gp.products && (
              <div key={gp.id}>
                {gp.match_label && <div className="guide-match-label">{gp.match_label}</div>}
                <ProductCard product={gp.products} />
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div className="container page">
      <div className="page-header">
        <div>
          <h1>Guias</h1>
          <p className="page-subtitle">Tutoriais para te ajudar a escolher certo</p>
        </div>
      </div>
