  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([cats, feat, news, brs, spot]) => {
        ...
        setNewProducts(news)
        setBrands(brs)
        setSpotlight(spot as SpotlightData | null)
        setCategoryCount(cats.length)
        setProductCount(feat.length + news.length)
    ...
    <div className="home-page">
      <section className="hero">
        <div className="hero-content">
          ...
          <p className="hero-subtitle">
            Reviews honestas, guias práticos e kits recomendados. Tudo que você precisa para cuidar do seu carro.
          </p>
          <div className="hero-stats">
            <div className="hero-stat">
              <Package size={28} weight="regular" className="hero-stat-icon" />
              <span className="hero-stat-num">{productCount}+</span>
              <span className="hero-stat-label">Produtos</span>
            </div>
            <div className="hero-stat">
              <Tag size={28} weight="regular" className="hero-stat-icon" />
              <span className="hero-stat-num">{brandCount}+</span>
              <span className="hero-stat-label">Marcas</span>
            </div>
            <div className="hero-stat">
              <UsersThree size={28} weight="regular" className="hero-stat-icon" />
      ...
      <section className="container section">
        <div className="section-header">
          <h2>Categorias</h2>
          <Link to="/busca" className="section-link">Ver tudo →</Link>
        </div>
            ...
            return (
              <Link key={cat.id} to={`/categoria/${cat.slug}`} className="cat-card">
                <div className="cat-card-icon-wrap">
                  <Icon size={32} weight="regular" className="cat-card-icon" />
                </div>
        ...
        <section className="container section">
          ...
          <div className="spotlight-card">
            <div className="spotlight-image">
              {spotlight.products.image_url && (
                <img src={spotlight.products.image_url} alt={spotlight.products.name} />
              )}
            </div>
            <div className="spotlight-body">
              ...
              <Link to={`/produto/${spotlight.products.slug}`} className="btn-primary">
                Ver produto
              </Link>
            </div>
          </div>
      ...
      {brands.length > 0 && (
        <section className="container section">
          <div className="section-header">
            ...
            <Link to="/marcas" className="section-link">Ver tudo →</Link>
          </div>
          <div className="brand-grid">
            {brands.map((b) => (
              <Link key={b.id} to={`/marca/${b.slug}`} className="brand-chip">
                {b.logo_url ? (
                  <img src={b.logo_url} alt={b.name} className="brand-chip-logo" />
                ) : (
